...
A projektben `Resendet` használunk az email kommunikáció kezelésére, mivel ez egy modern és megbízható megoldást kínál a tranzakciós és marketing emailek küldésére. A Resend mellett szóló főbb érvek:
...
- **Költséghatékonyság**: Az ingyenes tier (10,000 email/hó) megfelelő a legtöbb kezdeti igényre.
...
2. Hozz létre egy új API route-ot a `pages/api/email.ts` vagy `app/api/email/route.ts` alatt:
